...
public class EquipmentClassifyServicelmpl implements EquipmentClassifyService {
    ...
    public BackFrontMessage addEquipmentClassify(String classifyName) {
        Integer res=null;
        res= equipmentClassifyData.EquipmentClassifyIfExitByName(classifyName);
        if(res!=null){
    ...
    public BackFrontMessage updateEquipmentClassify(Integer classifyId, String newClassifyName) {
        EquipmentClassify equipmentClassify=equipmentClassifyData.getEquipmentClassifyById(classifyId);
        if(equipmentClassify==null){
            return new BackFrontMessage(500,"更新的设备分类不存在",null);
        }else{
            int res=0;
            res=equipmentClassifyData.updateEquipmentClassify(classifyId,newClassifyName);
            if(res==0){
    ...
    public BackFrontMessage deleteEquipmentClassify(Integer ClassifyId) {
        ...
        if(equipmentClassify==null){
            ...
            int res=0;
            res=equipmentClassifyData.deleteEquipmentClassify(ClassifyId);
            if(res==0){
    ...
    public BackFrontMessage getAllEquipmentClassify() {
        List<EquipmentClassify>equipmentClassifies=equipmentClassifyData.getAllEquipmentClassify();
